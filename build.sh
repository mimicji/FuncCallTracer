...
if [ -d $dynamorio_path ]; then
    echo "$0: Building CallTracer DynamoRIO tracer..."
    ...
    echo -e "#!/bin/bash\n$drrun_path -c $script_dir/build/libcalltracer.so -- \$@" > $script_dir/FuncCallTracer.sh
    chmod 777 $script_dir/FuncCallTracer.sh
    echo "$0: Done. Run \"./FuncCallTracer.sh <application>\" to start tracing."
